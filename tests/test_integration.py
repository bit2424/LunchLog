...
def test_complete_user_flow_with_restaurant_creation(auth_client, test_receipt_data, mock_google_places_data,celery_eager):
    """
    ...
    with mock.patch('apps.restaurants.tasks.GooglePlacesService') as MockService:
        ...
        mock_service_instance.fetch_restaurant_details.return_value = mock_google_places_data['place_details']
        mock_service_instance.find_place_from_text.return_value = mock_google_places_data['find_place']
        
        response = auth_client.post('/api/v1/receipts/', test_receipt_data, format='multipart')
...
@pytest.mark.integration
def test_complete_user_flow_with_existing_restaurant(auth_client, test_receipt_data, test_restaurant, test_image):
    """
    ...
    with mock.patch('apps.restaurants.tasks.update_restaurant_info.delay') as mock_task:
        response = auth_client.post('/api/v1/receipts/', receipt_data, format='multipart')
...
def test_user_can_list_their_receipts(auth_client, test_receipt_data, image_factory):
    ...
    with mock.patch('apps.restaurants.tasks.GooglePlacesService'):
        response = auth_client.post('/api/v1/receipts/', receipt_data_1, format='multipart')
        assert response.status_code == status.HTTP_201_CREATED
        response = auth_client.post('/api/v1/receipts/', receipt_data_2, format='multipart')
        assert response.status_code == status.HTTP_201_CREATED
...
def test_live_google_places_creates_and_updates_restaurant(db, external_api_key, celery_eager, jwt_client, test_image):
