...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not email:
            self.stdout.write(self.style.WARNING('DEFAULT_USER_EMAIL not set; skipping default user creation.'))
            return
        ...
        if created:
            ...
            if updates:
                ...
                self.stdout.write(self.style.SUCCESS(f'Updated default user: {email} ({", ".join(updates)})'))
            else:
                self.stdout.write(self.style.NOTICE(f'Default user already exists and is up-to-date: {email}'))
