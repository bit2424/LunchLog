...
def test_receipt_create_serializer_file_validation(test_receipt_data):
    ...
    large_file = SimpleUploadedFile(
        "large.jpg", file.getvalue(), content_type="image/jpeg"  # More than 1MB
    )
    data = test_receipt_data.copy()
    ...
    assert not serializer.is_valid()
    assert "image" in serializer.errors
    assert "The file you uploaded was either not an image or a corrupted image." in str(
    ...
    large_file = SimpleUploadedFile(
        "normal_image.tiff", file.getvalue(), content_type="image/tiff"  # More than 1MB
    )
    data = test_receipt_data.copy()
    data["image"] = large_file
    serializer = ReceiptCreateSerializer(data=data)
