...
class ReceiptSerializer(serializers.ModelSerializer):
    ...
    restaurant_id = serializers.UUIDField(
        source="restaurant.id", write_only=True, allow_null=True, required=False
    )
        ...
        if value is not None:
            if not Restaurant.objects.filter(id=value).exists():
                raise serializers.ValidationError(
                    "Restaurant with this ID does not exist."
                )
        return value
...
class ReceiptCreateSerializer(ReceiptSerializer):
    ...
    def validate_image(self, value):
        ...
        if int(value.size) > max_size:
            raise serializers.ValidationError(
                f"Image file too large MB). " "Maximum size allowed is 10 MB."
            )
