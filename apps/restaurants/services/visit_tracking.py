...
def update_visit_stats(user, restaurant, visit_date):
    ...
    with transaction.atomic():
        ...
        logger.info(f"Updated restaurant visit: {user.email} -> {restaurant.name} "
        ...
        for cuisine in restaurant_cuisines:
            ...
            logger.info(f"Updated cuisine stat: {user.email} -> {cuisine.name} "
...
def get_user_restaurant_stats(user, limit=None):
    ...
    """
    queryset = UserRestaurantVisit.objects.filter(user=user).select_related('restaurant')
...
def get_user_top_restaurants(user, limit=10):
    ...
    """
    visits = UserRestaurantVisit.objects.filter(user=user).select_related('restaurant')[:limit]
