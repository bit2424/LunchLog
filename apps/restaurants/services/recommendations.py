...
class RestaurantRecommendationService:
    ...
    def get_user_frequent_locations(self, user, limit: int = 5) -> List[Dict]:
        ...
        """
        top_visits = UserRestaurantVisit.objects.filter(
            user=user,
            restaurant__latitude__isnull=False,
            restaurant__longitude__isnull=False
        ).select_related('restaurant').order_by('-visit_count')[:limit]
    ...
    def get_user_top_cuisines(self, user, limit: int = 5) -> List[str]:
        ...
        """
        top_cuisines = UserCuisineStat.objects.filter(
            user=user
        ).select_related('cuisine').order_by('-visit_count')[:limit]
        
        return [stat.cuisine.name for stat in top_cuisines]
    
    def get_good_restaurants_recommendations(self, user, limit: int = 20,
                                             radius: int = 2000, per_location_limit: int = 20) -> List[Dict]:
        """
        ...
        for location in frequent_locations:
            try:
                ...
                logger.error(f"Error getting good restaurant recommendations near {location['restaurant_name']}: {str(e)}")
                continue
    ...
    def get_cheap_restaurants_recommendations(self, user, limit: int = 20,
                                              radius: int = 2000, per_location_limit: int = 20) -> List[Dict]:
        """
        ...
        for location in frequent_locations:
            try:
                ...
                logger.error(f"Error getting cheap restaurant recommendations near {location['restaurant_name']}: {str(e)}")
                continue
    ...
    def get_cuisine_match_recommendations(self, user, limit: int = 20,
                                          radius: int = 2000, per_location_limit: int = 20) -> List[Dict]:
        """
        ...
        for location in frequent_locations:
            try:
                ...
                for rec in recommendations:
                    ...
                    rec['matched_cuisines'] = self._get_matching_cuisines(rec.get('cuisines', []), user_cuisines)
                
                all_recommendations.extend(recommendations)
                
            except Exception as e:
                logger.error(f"Error getting cuisine-based recommendations near {location['restaurant_name']}: {str(e)}")
                continue
    ...
    def _get_matching_cuisines(self, restaurant_cuisines: List[str], user_cuisines: List[str]) -> List[str]:
        """
    ...
    def get_all_recommendations(self, user, limit_per_type: int = 10,
                                radius: int = 2000, per_location_limit: int = 20) -> Dict[str, List[Dict]]:
        """
        ...
        """
        return {
            'good': self.get_good_restaurants_recommendations(user, limit_per_type, radius, per_location_limit),
            'cheap': self.get_cheap_restaurants_recommendations(user, limit_per_type, radius, per_location_limit),
            'cuisine_match': self.get_cuisine_match_recommendations(user, limit_per_type, radius, per_location_limit)
        }
