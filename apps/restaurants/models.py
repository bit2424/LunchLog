...
class Restaurant(models.Model):
    ...
    place_id = models.CharField(
        max_length=255, unique=True, help_text="Google Places API place_id"
    )
    name = models.CharField(max_length=255, help_text="Restaurant name")
    ...
    longitude = models.FloatField(
        blank=True, null=True, help_text="Longitude coordinate"
    )
...
class UserRestaurantVisit(models.Model):
    ...
    restaurant = models.ForeignKey(
        Restaurant, on_delete=models.CASCADE, related_name="user_visits"
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(
                fields=["user", "-visit_count"], name="user_rest_visit_count_idx"
            ),
            models.Index(
    ...
    def __str__(self):
        return (
            f"{self.user.email} -> {self.restaurant.name} ({self.visit_count} visits)"
        )
...
class UserCuisineStat(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="cuisine_stats"
    )
    cuisine = models.ForeignKey(
        Cuisine, on_delete=models.CASCADE, related_name="user_stats"
    )
    visit_count = models.PositiveIntegerField(
        default=0,
