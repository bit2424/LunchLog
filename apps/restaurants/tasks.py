...
def update_restaurant_info(self, restaurant_id: str):
    ...
    try:
        ...
        logger.info(f"Updating restaurant info for {restaurant.name} (ID: {restaurant_id})")
        ...
        if restaurant.place_id.startswith('stub_'):
            logger.info(f"Found stub restaurant {restaurant.name}, searching for real place_id")
            ...
            if place_candidate and place_candidate.get('place_id'):
                logger.info(f"Found real place_id {place_candidate['place_id']} for stub {restaurant.name}")
                restaurant.place_id = place_candidate['place_id']
                restaurant.save(update_fields=['place_id'])
            else:
                logger.warning(f"Could not find real place_id for stub restaurant {restaurant.name}")
                return {
                    'status': 'error',
        ...
        with transaction.atomic():
            ...
            logger.info(f"Restaurant {restaurant_id} updated. Fields changed: {updated_fields}")
        ...
        if self.request.retries < self.max_retries:
            ...
            logger.info(f"Retrying task in {countdown} seconds (attempt {self.request.retries + 1})")
...
@shared_task
def create_restaurant_from_places_data(place_id: str, name: str, address: str) -> Optional[str]:
    """
